...
try {
    if ($method === 'GET') {
        echo json_encode(['data' => FinancialService::listExpenses()]);
    } elseif ($method === 'POST') {
        $payload = json_decode(file_get_contents('php://input'), true) ?? [];
        $user = Auth::currentUser();
        if (!$user) { http_response_code(401); echo json_encode(['error'=>'unauthorized']); exit; }
        echo json_encode(['data' => FinancialService::addExpense($payload, $user['id'])]);
    } elseif ($method === 'DELETE') {
        $user = Auth::currentUser();
        if (!$user) { http_response_code(401); echo json_encode(['error'=>'unauthorized']); exit; }
        $id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
        echo json_encode(['deleted' => FinancialService::deleteExpense($id)]);
    } else {
